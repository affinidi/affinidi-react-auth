...
const errorMessageMap: Record<string, string> = {
  invalid_request:
    "Invalid authorization request or We could not find any data that meets our criteria.",
...
const errorDescriptionMessageMap: Record<string, string> = {
  ...
  missing_concierge_data:
    "Missing Concierge data, please try again in few minutes",
  request_declined: "Request has been declined by user",
  user_declined_connecting_missing_connectors:
    "User has declined to connect missing connectors",
};
      ...
      if ("error" in response) {
        ...
        setErrorDescription(
          (response.errorDescription &&
            errorDescriptionMessageMap[response.errorDescription]) ||
            undefined
        );
        return;
      } else {
